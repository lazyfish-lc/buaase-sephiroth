...
public class EnvironmentBigObject : BigObject 
{
    ...
    {
        // 逻辑不受地图禁用影响
        if (changedChild == staticData.pastObject) 
        {
            Debug.Log("BigObject 捕获到过去地图中的状态变化");
            
            // 玩法3：过去影响现在
            if (((TreeSmallObject) staticData.pastObject).treeState.isDestroyed) 
            {
                // 即使 presentObject (门) 所在的地图当前是禁用的
                // 我们依然可以修改它的数据状态
                if (staticData.presentObject is DoorSmallObject door) 
                {
                    ((DoorSmallObject) door).doorState.isDestroyed = true; 
